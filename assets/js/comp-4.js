...
let currentPlayerIndex = 0; // Jugador actual
let shotsInRound = 0; // Contador de disparos en la ronda
...
function updateShot(hit) {
    ...
    let cell = document.getElementById(`shot-${currentPlayerIndex}-${player.currentShot}`);
    ...
    document.getElementById(`score-${currentPlayerIndex}`).textContent = player.score;
    ...
    // Si ya hizo 4 disparos, pasamos al siguiente jugador
    if (shotsInRound >= 4 || player.currentShot >= totalShots) {
        moveToNextPlayer();
    }
}

function moveToNextPlayer() {
    shotsInRound = 0; // Reiniciar el contador de la ronda

    // Buscar al siguiente jugador que no haya terminado
    let found = false;
    for (let i = 1; i <= players.length; i++) {
        let nextIndex = (currentPlayerIndex + i) % players.length;
        if (players[nextIndex].currentShot < totalShots) {
    ...
    if (!found) {
        checkGameEnd(); // Si no hay más jugadores, termina el juego
    }
}
